...
class LoginViewController: UIViewController {
    ...
    // MARK: - UI Components
    private let headerView = AuthHeaderView(title: "Bienvenido", subTitle: "Inicia sesión en tu cuenta")
    
    private let emailField = CustomTextField(fieldType: .email)
    private let passwordField = CustomTextField(fieldType: .password)
    
    private let signInButton = CustomButton(title: "Iniciar sesión", backgroundColor: .blue, fontSize: .big)
    private let newUserButton = CustomButton(title: "¿Aún no tienes una cuenta? ¡Regístrate!", fontSize: .medium)
    private let forgotPasswordButton = CustomButton(title: "¿Olvidaste la contraseña?", fontSize: .small)
